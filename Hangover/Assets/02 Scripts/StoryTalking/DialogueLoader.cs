...
public struct DialogueEntry
{
    ...
    public List<int> nextDialogueIds; // 다음 대사 ID 리스트 (복수 가능)
    ...
    public static List<DialogueEntry> LoadDialogues(string filePath)
    {
        ...
        if (File.Exists(filePath))
        {
            ...
            for (int i = 1; i < lines.Length; i++)
            {
                        ...
                        int.TryParse(columns[1].Trim(), out int day))
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(columns[5]))
                        {
                            ...
                            foreach (var idString in idStrings)
                            {
                                ...
                                {
                                    Debug.LogWarning($"유효하지 않은 다음 대사 ID: '{idString.Trim()}' at line {i + 1}");
                                }
                            }
                        }
                ...
                {
                    Debug.LogWarning($"데이터 형식이 올바르지 않습니다. 최소 7개의 컬럼이 필요합니다. at line {i + 1}");
                }
            }
        }
        else
