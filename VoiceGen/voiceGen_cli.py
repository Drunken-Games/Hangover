...
for idx, item in enumerate(char_list):
    ...
    char_sounds[item] = AudioSegment.from_mp3(f'./sources/{str_idx}.mp3')
    char_sounds_low[item] = AudioSegment.from_mp3(f'./sources/low/{str_idx}.mp3')
    char_sounds_high[item] = AudioSegment.from_mp3(f'./sources/high/{str_idx}.mp3')
...
while True:
    ...
    for _, row in df.iterrows():
        ...
        for ch in text:
            ...
            char_sound_low = char_sounds_low[jamo_ch[0]]
            ...
            pitch_char_sound_low = char_sound_low._spawn(char_sound_low.raw_data, overrides={'frame_rate': new_sample_rate})
            result_sound_low = pitch_char_sound_low if result_sound_low is None else result_sound_low + pitch_char_sound_low
        ...
        filename_low = f'{result_folder}/day{day}_{id}_{npcId}_low.mp3'
        ...
        result_sound_low.export(filename_low, format='mp3')
        result_sound_high.export(filename_high, format='mp3')
        print(f"저장 완료: {filename_normal}, {filename_low}, {filename_high}")
